...
function NotificationCard({ visited, senderName, senderTeamName, message, createdAt }:
    { visited: boolean, senderName: string, senderTeamName: string, message: string, createdAt: Date }) {
    return (
        <>
            <div className={`${notificationCardStyle(visited)} p-3 border border-gray-200 rounded-sm mt-0.5 shadow-sm transition-colors duration-200`}>
